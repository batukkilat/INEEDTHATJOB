...
# Windows: create a file named .env in the app folder using Notepad
# Mac/Linux:
...
Needed only to send applications automatically. You can skip this and apply manually. See the in-app setup guide for Gmail/Outlook steps.
...
Run `pip install -e .` again. For database errors, delete the `.db` file in `data/` and restart — the app recreates it (existing data will be lost).
